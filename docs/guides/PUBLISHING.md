...
Decoder-only CERPT Base checkpoint는 별도 kind로 업로드한다.

```powershell
python scripts/upload_model.py `
  --model-dir artifacts/cerpt-causal-korean-smoke `
  --repo-id <HF_USER>/cerpt-causal-korean-smoke `
  --kind causal
```

현재 causal checkpoint는 PyTorch/Transformers용 연구 모델이다. 실제 vLLM이나 Ollama에서 일반 모델처럼 쓰려면 KV cache, custom backend, 변환·양자화 경로를 추가로 구현해야 한다.
...
현재 멀티모달 checkpoint는 구조 연결용이다. 이미지·비디오 데이터로 추가학습하기 전에는 시각 응답 성능을 보장하지 않는다. 이 사실은 [MODEL_CARD.md](../model-cards/MODEL_CARD.md)에 명시한다.

...
def main() -> None:
    ...
    parser.add_argument("--tokenizer-dir", default=None, help="use an already trained tokenizer instead of building a tiny corpus tokenizer")
    ...
    architecture = json.loads(Path(args.architecture_config).read_text(encoding="utf-8")) if args.architecture_config else {}
    ...
    (output_dir / "training_config.json").write_text(json.dumps({**vars(args), "operators": OPERATORS}, ensure_ascii=False, indent=2), encoding="utf-8")
